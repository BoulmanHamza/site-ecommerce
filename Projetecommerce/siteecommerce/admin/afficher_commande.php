...
if(isset($_POST['valider'])){
 $req="UPDATE details set status=1 where id_destinataire=?";
 $stmt=$conn->prepare($req);
  $stmt->execute([$_SESSION['voir']]);
  $err='<div class="alert alert-success alert-dismissible">
                          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                          <h4><i class="icon fa fa-warning"></i>la commande est valider avec success </h4>
                      </div> ';
}
...
$tabanimation='<table id="example1" class="table table-bordered ">
          ...
          $req="SELECT det.status as 'statusdetails' ,p.Nom as 'nomproduit',tva,Prix,quantity,categorie_produit,photo1,des.id_destinataire as 'id_destinataire' from details det,produit p,destinataire des where p.id_produit=det.id_produit and det.id_destinataire=des.id_destinataire and des.id_destinataire=? and det.status=0";
          $stmt=$conn->prepare($req);
          $stmt->execute([$_SESSION['voir']]);
          ...
          foreach ($stmt as $row) {
            ...
            $status='<span class="label label-warning">pas encore</span>';
  ...
  <div class="col-lg-6 col-sm-12">
    ...
    <section class="content">
      ...
      <form method="POST" action="">
       <button class='btn btn-success btn-sm edit btn-flat' name="valider"><i class='fa fa-check'></i> Valider</button> 
       </form>
    </section>
