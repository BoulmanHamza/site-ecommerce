  ...
  if(!empty($_POST['nomcategorie']))
            {
              ...
              $nbr=0;
              $reqcateg="select * from categorie_produit where categorie_produit=?";
              ...
              if($nbr==0){
                $req="insert into categorie_produit(categorie_produit) values(?)";
...
<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
      ...
      <h1>
       Liste des classes de produits
      </h1>
      <ol class="breadcrumb">
        <li><a href="home.php"><i class="fa fa-dashboard"></i> Acceuil</a></li>
        <li class="">Produit</li>
		<li class="active">Catégorie des Produits</li>
      </ol>
    </section>
...
$c=0;
if(isset($_POST['sup'])){
  $sql2="DELETE from categorie_produit where id_categorie_produit=? and categorie_produit NOT in (SELECT categorie_produit from produit)";
  $stmt2=$conn->prepare($sql2);
  $stmt2->execute([$_POST['sup']]);
   $sql3="select * from categorie_produit where id_categorie_produit=?";
...
$tabcateg='<table id="example1" class="table table-bordered ">
          ...
          foreach ($stmt as $row) {
            if($row['categorie_produit']=='Accessoire Holographique' or $row['categorie_produit']=='Hologramme Particulier' or $row['categorie_produit']=='Hologramme Professionnel'){
              $tabcateg=$tabcateg.'
                    <tr>
                   ...
                   $tabcateg=$tabcateg.'
                    <tr>
                    <td>'.$row['categorie_produit'].'</td>
                     <td> <button class="btn btn-danger btn-sm delete btn-flat" onclick="aff(this)" data-toggle="modal" data-target="#sup_categ" id_categ="'.$row['id_categorie_produit'].'"><i class="fa fa-trash"></i> Supp</button>
                    </td>
                  </tr>';
...
function aff(val){
              var str=val.getAttribute("id_categ");
               var xmlhttp;
                          ...
                          xmlhttp.open("get","ajax/select_categ_produit.php?id_categ="+str,true);
                          xmlhttp.send();
